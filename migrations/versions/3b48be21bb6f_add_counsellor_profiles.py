...
def upgrade():
    if not _table_exists('counsellor_profiles'):
    ...
    if not _index_exists('ix_counsellor_profiles_account_id'):
    ...
    if not _index_exists('ix_counsellor_profiles_verification_status'):
        with op.batch_alter_table('counsellor_profiles', schema=None) as batch_op:
