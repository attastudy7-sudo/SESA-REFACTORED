...
def upgrade():
    ...
    if not _index_exists('ix_question_test_type'):
        op.create_index('ix_question_test_type', 'question', ['test_type'], unique=False)

    # ── school ────────────────────────────────────────────────
    if not _table_exists('school'):
        op.create_table(
            'school',
            sa.Column('id', sa.Integer(), nullable=False),
    ...
    if not _index_exists('ix_school_access_code'):
        op.create_index('ix_school_access_code', 'school', ['access_code'], unique=True)
    if not _index_exists('ix_school_qr_token'):
    ...
    if not _index_exists('ix_accounts_email'):
        op.create_index('ix_accounts_email', 'accounts', ['email'], unique=True)
    if not _index_exists('ix_accounts_username'):
        op.create_index('ix_accounts_username', 'accounts', ['username'], unique=True)
    if not _index_exists('ix_accounts_school_id'):
